...
import javax.swing.*;
public class BattleshipDisplay extends JPanel{
   ...
   * make sure "ok" button is not enabled if the ship position is illegal
   */
   public BattleshipDisplay(JFrame frame)
   {
      //playerNum = p;
      ...
      label.setText("Player 1, Add your " + shipNum + "-pegged ship. Use the 'change direction' button to change from vertical to horizontal. Don't let your opponent see! Press confirm when finished.");
   ...
   private class next implements ActionListener {
      public void actionPerformed(ActionEvent e) {
         ...
         if(gameOver == 4) {
            ...
            endFrame.setContentPane(new EndPanel(endFrame,2));
            endFrame.setSize(500,500);
         }
         else {
            frame.setVisible(false);
            label.setText("Player 2's turn. Click the spot you wish to target!");
            if(newFrame==null){
               newFrame = new JFrame("Battleship Frame");
               newFrame.setVisible(true);
               newFrame.setLocation(0,0);
               newFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
               newFrame.setContentPane(new BattleshipDisplay2(frame, newFrame));
   ...
   /*public void setNextEnabled(boolean arg) {
      nextEnabled = arg;
   }
   public boolean getNextEnabled() {
      return nextEnabled;
   }
   public void setGameOver(boolean arg) {
      gameOver = arg;
   }
